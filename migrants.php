...
// ДИАГНОСТИКА: Проверяем данные сессии
echo "<!-- Диагностика:\n";
echo "session_id: " . session_id() . "\n";
echo "user_id: " . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'не установлен') . "\n";
echo "user_type: " . (isset($_SESSION['user_type']) ? $_SESSION['user_type'] : 'не установлен') . "\n";
echo "isLoggedIn(): " . (isLoggedIn() ? 'true' : 'false') . "\n";
echo "isAdmin(): " . (isAdmin() ? 'true' : 'false') . "\n";
echo "-->\n";

if (!isLoggedIn()) {
    echo "<!-- Не авторизован, редирект на login.php -->\n";
    header('Location: login.php');
    exit;
}

if (!isAdmin()) {
    echo "<!-- Не админ, редирект на index.php -->\n";
